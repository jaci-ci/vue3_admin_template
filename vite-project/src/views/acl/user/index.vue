...
const getHasUser = async (pager = 1) => {
    ...
    try {
        ...
        if (result.code == 200) {
            ...
            ElMessage({ type: 'error', message: result.message || '获取用户数据失败' });
        }
    } catch (error: any) {
        // 响应拦截器已经处理了业务错误码(206/207)并弹出提示，这里处理其他异常
        // 兼容多种错误形态：error.response?.data、error.data、error.message、error本身
        const responseData = error.response?.data || error.data || {};
        const errorMsg = responseData.msg || responseData.message || responseData.error || error.message || error || '网络请求失败，请检查接口连接';
        // 如果是业务错误，响应拦截器已经弹出提示，这里不再重复
        if (!responseData.code || responseData.code !== 206 && responseData.code !== 207) {
...
const save = async () => {
    ...
    if (result.code == 200) {
        ...
        drawer.value = false;
        //提示消息
        ElMessage({ type: 'error', message: result.message || (userParams.id ? '更新失败' : '添加失败') });
    }
}
//取消按钮的回调
const cancel = () => {
